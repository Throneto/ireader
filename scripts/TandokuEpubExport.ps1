...
function GenerateEpub($markdownFiles, [string]$targetPath, [string]$title) {
    ...
    pandoc $markdownFiles -f commonmark+footnotes -o $tempEpub -t epub3 --metadata title="$title" --metadata author="ireadervalar" --metadata lang="$($volume.definition.language)"
    Pop-Location
